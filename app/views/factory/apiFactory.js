...
angular.module('bmpUiApp')
  .factory('apiFactory', function($http, $q, $location, $cookies, ConfigService) {
    ...
    apiFactory.getWhoIsWhereASNLike = function(asn, lim) {
      ...
      return $http.get(urlBase + "whois/asn?where=w.asn%20like%20%27" + asn +
        "%%27" + lim);
    };

    apiFactory.getWhoIsWhereASNLikeCOUNT = function(asn) {
      return $http.get(urlBase + "whois/asn/count?where=w.asn%20like%20%27" +
        asn + "%%27");
    };
    ...
    apiFactory.getWhoIsName = function(name, lim) {
      ...
      var uri = urlBase + "whois/asn?where=w.as_name like '%" + name +
        "%' or w.org_name like '%" + name + "%'&limit=" + lim;
    ...
    apiFactory.getWhoIsASNameCOUNT = function(name) {
      var uri = urlBase + "whois/asn/count?where=w.as_name like '%" + name +
        "%' or w.org_name like '%" + name + "%'";
      var res = encodeURI(uri);
      return $http.get(res);
    };
    ...
    apiFactory.getPeerHistory = function(peerHashId, amount_of_entries) {
      return $http.get(urlBase + "peer/prefix/" + peerHashId + '?last=' +
        amount_of_entries);
    };
    ...
    apiFactory.getPeerRibLookup = function(peer_hash_id, ip) {
      return $http.get(urlBase + "rib/peer/" + peer_hash_id + "/lookup/" +
        ip);
    };

    apiFactory.getPeerRibPrefix = function(peer_hash_id, ip) {
      return $http.get(urlBase + "rib/peer/" + peer_hash_id + "/prefix/" +
        ip);
    };
    ...
    apiFactory.getUpdatesOverTimeByPeer = function(peer_hash_id) {
      return $http.get(urlBase + "updates/peer/" + peer_hash_id +
        "/top/interval/5");
    };
    ...
    apiFactory.getWithdrawsOverTimeByPeer = function(peer_hash_id) {
      return $http.get(urlBase + "withdrawns/peer/" + peer_hash_id +
        "/top/interval/5");
    };
    ...
    //For Tops view
    apiFactory.getTopUpdates = function(searchPeer, searchPrefix, groupBy,
      startTimestamp, endTimestamp, joinWhoisPrefix) {
      joinWhoisPrefix = joinWhoisPrefix || false;
      return $http.get(urlBase + "updates/top?searchPeer=" + searchPeer +
        "&searchPrefix=" + searchPrefix + "&groupBy=" + groupBy +
        "&joinWhoisPrefix=" + joinWhoisPrefix + "&startTs=" +
        startTimestamp + "&endTs=" + endTimestamp)
    };

    apiFactory.getTopWithdraws = function(searchPeer, searchPrefix, groupBy,
      startTimestamp, endTimestamp, joinWhoisPrefix) {
      joinWhoisPrefix = joinWhoisPrefix || false;
      return $http.get(urlBase + "withdrawns/top?searchPeer=" + searchPeer +
        "&searchPrefix=" + searchPrefix + "&groupBy=" + groupBy +
        "&joinWhoisPrefix=" + joinWhoisPrefix + "&startTs=" +
        startTimestamp + "&endTs=" + endTimestamp)
    };

    apiFactory.getUpdatesOverTime = function(searchPeer, searchPrefix,
      seconds, startTimestamp, endTimestamp) {
      return $http.get(urlBase + "updates/trend/interval/" + seconds +
        "?searchPeer=" + searchPeer + "&searchPrefix=" + searchPrefix +
        "&startTs=" + startTimestamp + "&endTs=" + endTimestamp)
    };

    apiFactory.getWithdrawsOverTime = function(searchPeer, searchPrefix,
      seconds, startTimestamp, endTimestamp) {
      return $http.get(urlBase + "withdrawns/trend/interval/" + seconds +
        "?searchPeer=" + searchPeer + "&searchPrefix=" + searchPrefix +
        "&startTs=" + startTimestamp + "&endTs=" + endTimestamp)
    };
    ...
    apiFactory.getORRospf = function(peerHashId, routerId) {
      return $http.get(urlBase + "orr/peer/" + peerHashId + "/ospf/" +
        routerId);
    };

    apiFactory.getORRisis = function(peerHashId, routerId) {
      return $http.get(urlBase + "orr/peer/" + peerHashId + "/isis/" +
        routerId);
    };
    ...
    apiFactory.getSPFospf = function(peerHashId, routerId) {
      return $http.get(urlBase + "linkstate/spf/peer/" + peerHashId +
        "/ospf/" + routerId);
    };
    ...
    apiFactory.getGeoIPList = function(page, limit, whereClause, sort) {
      return $http.get(urlBase + "geoip/get/" + page + "/" + limit +
        "?where=" + whereClause + (sort ? ("&sort=" + sort.name +
    ...
    apiFactory.updateGeoIP = function(ip_start, col, value) {
      return $http.get(urlBase + "geoip/update/" + ip_start + "/" + col +
        "/" + value);
    };
    ...
    apiFactory.getGeoLocationList = function(page, limit, whereClause, sort) {
      return $http.get(urlBase + "geolocation/get/" + page + "/" + limit +
        "?where=" + whereClause + (sort ? ("sort=" + sort.name +
          "&sortDirection=" + sort.sort.direction) : ""));
    };

    apiFactory.getGeoLocationCount = function(whereClause) {
      return $http.get(urlBase + "geolocation/getcount" + "?where=" +
        whereClause);
    };
    ...
    apiFactory.updateGeoLocation = function(country, city, col, value) {
      return $http.get(urlBase + "geolocation/update/" + country + "/" +
        city + "/" + col + "/" + value);
    };

    apiFactory.deleteGeoLocation = function(country, city) {
      return $http.get(urlBase + "geolocation/delete/" + country + "/" +
        city);
    };
    ...
    // security analysis
    apiFactory.getMisMatchPrefix = function(page, limit, sort, desc, asn,
      prefix, where) {
