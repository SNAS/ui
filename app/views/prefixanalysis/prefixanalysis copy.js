...
angular.module('bmpUiApp')
  .controller('PrefixAnalysisController',['$scope', 'apiFactory', '$http', '$timeout', '$interval','$location', '$window','$anchorScroll',function ($scope, apiFactory, $http, $timeout,$window, $location, $anchorScroll) {
    ...
    $scope.getAsPathChanged = function(allHisData)
    {
      ...
      else
      {
        ...
        $scope.asPathChangePrecent = new Array(24);
        var j = 0;
        while(j<24)
        {
  ...
  .directive('d3Directive',function(){
    function link($scope,element){
      var w = 600;
      var h = 20;
      var data = [5, 10, 13, 19, 21, 25, 22, 18, 15, 13, 11, 12, 15, 20, 18, 17, 16, 18, 23, 25, 21, 22, 23, 24];
      if($scope.asPathChangePrecent === undefined)
      {
        data = [5, 10, 13, 19, 21, 25, 22, 18, 15, 13, 11, 12, 15, 20, 18, 17, 16, 18, 23, 25, 21, 22, 23, 24];
      }
      else
      {
        data = $scope.asPathChangePrecent;
      }
...
//create the last modified time
      ...
      svg2.selectAll("rect")
        ...
        .attr("style",function(d, i) {
          console.log(d);
          var color = d3.scale.linear().domain([0,30]).range(['#848AA8','#855EF9']);
          console.log(i);
          console.log(color(d));
          return "fill:" +color(d);
        })
